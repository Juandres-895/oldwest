...
export const CategoryPage = memo(
  ...
  >(function CategoryPage(
    ...
    return (
      ...
      >
        <div className="flex-1 px-5 pb-4 pt-8 sm:px-7">
          <h2 className="font-heading text-2xl text-brass-light sm:text-3xl">
            {pick(category.nameEs, category.nameEn)}
            {partCount > 1 && (
              <span className="ml-2 font-ui text-sm font-normal text-bone-muted">
                ({partIndex + 1}/{partCount})
              </span>
            )}
          </h2>
          {partIndex === 0 && (category.taglineEs || category.taglineEn) && (
